# Gauntlet: Dark Legacy Setup Guide
...
Start ``ArchipelagoLauncher.exe`` and choose ``GDL Client``. You will be asked to provide a ``.apGDL`` patch file so
...
The Client will automatically try to connect to Dolphin every 5 seconds and will do so if GDL is running. If this
